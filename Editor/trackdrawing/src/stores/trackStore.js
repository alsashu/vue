...
export const useTrackStore = defineStore("trackStore", () => {
  ...
  const startDrawing = (mode = "draw") => {
    // Finish current track before starting new mode
    finishCurrentTrack();
  ...
  const setDrawingMode = (mode) => {
    // Always finish current track when switching modes
    if (drawingMode.value === "polyline" && currentTrack.value) {
      finishCurrentTrack();
    }
    drawingMode.value = mode;

    // Clear current track only if switching away from drawing modes
    if (mode !== "draw" && mode !== "polyline") {
      currentTrack.value = null;
    }

    // Clear selections when entering drawing modes
    if (mode === "draw" || mode === "polyline") {
      selectedTrack.value = null;
      selectedComponent.value = null;
    }
